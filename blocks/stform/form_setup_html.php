...
$bt = BlockType::getByHandle('stform');
$addSelected = true;
?>
<p>
...
<div class="ccm-tab-content" id="ccm-tab-content-form-options">
	...
	<fieldset>
		...
		<div class="form-group">
            <?php echo $form->label('titleInForm', 'メール冒頭に記載する敬称（表示例：〜様、%s部分が置換されます。）')?>
            <?php echo $form->text('titleInForm', $this->controller->titleInForm)?>
        </div>

        <div class="form-group">
            <?php echo $form->label('titleInFormKey', 'メール冒頭に記載するキー')?>
            <?php echo $form->text('titleInFormKey', $this->controller->titleInFormKey)?>
        </div>

        <div class="form-group">
			<?php echo $form->label('clientSubject', 'クライアント用メール件名（%sを入れるとフォーム名称に置換されます。）')?>
			<?php echo $form->text('clientSubject', $this->controller->clientSubject)?>
		</div>

		<div class="form-group">
			<?php echo $form->label('clientMessBefore', 'クライアント用メール前文')?>
			<?php echo $form->textarea('clientMessBefore', $this->controller->clientMessBefore, array('rows' => 3))?>
		</div>

		<div class="form-group">
			<?php echo $form->label('clientMessAfter', 'クライアント用メール後文')?>
			<?php echo $form->textarea('clientMessAfter', $this->controller->clientMessAfter, array('rows' => 3))?>
		</div>

		<div class="form-group">
			<?php echo $form->label('adminSubject', '管理者用メール件名（%sを入れるとフォーム名称に置換されます。）')?>
			<?php echo $form->text('adminSubject', $this->controller->adminSubject)?>
		</div>

		<div class="form-group">
			<?php echo $form->label('adminMessBefore', '管理者用メール前文')?>
			<?php echo $form->textarea('adminMessBefore', $this->controller->adminMessBefore, array('rows' => 3))?>
		</div>

		<div class="form-group">
			<?php echo $form->label('adminMessAfter', '管理者用メール後文')?>
			<?php echo $form->textarea('adminMessAfter', $this->controller->adminMessAfter, array('rows' => 3))?>
		</div>
...
<div class="ccm-tab-content" id="ccm-tab-content-form-add">
	...
	<fieldset id="newQuestionBox">
		...
		<div class="form-group">
			<?php echo $form->label('addText', '備考テキスト')?>
			<?php echo $form->textarea('addText', array('rows' => 3))?>
		</div>
		<div class="form-group">
...
<div class="ccm-tab-content" id="ccm-tab-content-form-edit">
	...
	<div id="editQuestionForm" style="display:none">
		<fieldset>
			...
			<div class="form-group">
				<?php echo $form->label('addTextEdit', '備考テキスト')?>
				<?php echo $form->textarea('addTextEdit', array('rows' => 3))?>
			</div>
			<div class="form-group">
